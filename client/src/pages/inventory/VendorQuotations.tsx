...
export default function VendorQuotations() {
  ...
  const handleCreateReceivedQuote = () => {
    ...
    const quotationData = {
      ...
      quotationDate: new Date().toISOString(),
      validUntil: rfqValidUntil ? new Date(rfqValidUntil).toISOString() : null,
      subject: rfqProject ? `Quotation for Project/MR: ${rfqProject}` : "Vendor Quotation",
      totalAmount: totalAmount.toString(),
      status: "received",
      notes: rfqNotes,
      senderId: rfqVendor,
    ...
    createRFQMutation.mutate(quotationData, {
      onSuccess: () => {
        setIsReceivedQuoteDialogOpen(false);
        // Note: resetForm is already called in createRFQMutation.onSuccess
      }
    });
  };
    ...
    <div className="p-8 space-y-8 animate-in fade-in duration-500">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="space-y-8">
        <TabsList className="bg-slate-100/50 p-1 border border-slate-200/60 rounded-xl">
          <TabsTrigger 
            value="request" 
            className="rounded-lg px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm transition-all"
          >
            Request Quotation
          </TabsTrigger>
          <TabsTrigger 
            ...
            className="rounded-lg px-6 py-2.5 data-[state=active]:bg-white data-[state=active]:text-indigo-600 data-[state=active]:shadow-sm transition-all"
          >
            Received Quote Analysis
          </TabsTrigger>
        </TabsList>

        <TabsContent value="request" className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
        ...
        <TabsContent value="analysis" className="space-y-8 animate-in fade-in slide-in-from-bottom-2 duration-500">
          <div className="space-y-4">
            <div className="flex items-center justify-between gap-4">
              <div className="relative flex-1">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-muted-foreground" />
                <Input 
                  placeholder="Search received quotations..." 
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="pl-10 bg-muted/30 border-none h-11 shadow-none focus-visible:ring-1 focus-visible:ring-indigo-500"
                />
                  ...
                  {isLoading ? (
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-12 text-muted-foreground italic">
                        No received quotations found.
                      </TableCell>
                    </TableRow>
                  ) : receivedQuotations.map((q: any) => (
                    <TableRow key={q.id} className="hover:bg-slate-50/50 transition-colors border-slate-100">
                      <TableCell className="font-bold text-indigo-600 py-4">
                        {q.quotationNumber}
                      </TableCell>
      ...
      <Dialog open={isReceivedQuoteDialogOpen} onOpenChange={setIsReceivedQuoteDialogOpen}>
        <DialogContent className="max-w-3xl p-0 overflow-hidden border-none shadow-2xl rounded-xl">
          <DialogHeader className="px-5 py-3 bg-slate-50 border-b flex flex-row items-center justify-between space-y-0">
            ...
            <Button variant="ghost" size="icon" className="h-8 w-8 rounded-full" onClick={() => setIsReceivedQuoteDialogOpen(false)}>
              <X className="h-4 w-4" />
            </Button>
          </DialogHeader>
          ...
          <DialogFooter className="px-5 py-3 bg-slate-50 border-t flex flex-row justify-end space-x-2">
            ...
            <Button onClick={handleCreateReceivedQuote} className="bg-emerald-600 hover:bg-emerald-700 text-white px-6 h-9 text-sm font-bold">
              Log Quotation
            </Button>
          </DialogFooter>
        </DialogContent>

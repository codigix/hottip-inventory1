...
import {
  insertLogisticsShipmentSchema,
  insertLogisticsStatusUpdateSchema,
  insertLogisticsCheckpointSchema,
  updateLogisticsShipmentSchema,
  logisticsShipmentFilterSchema,
  updateLogisticsShipmentStatusSchema,
  closePodUploadSchema,
  insertLogisticsAttendanceSchema,
  updateLogisticsAttendanceSchema,
  logisticsCheckInSchema,
  logisticsCheckOutSchema,
  attendancePhotoUploadSchema,
  insertLogisticsTaskSchema,
...
export const getLogisticsShipment = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const deleteLogisticsShipment = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const updateShipmentStatus = async (
  ...
  try {
    ...
    if (!canUpdate) {
      res
        .status(403)
        .json({ error: "Not authorized to update this shipment's status" });
      return;
    }
...
export const getShipmentTimeline = async (
  ...
  try {
    ...
    if (
      ...
      res
        .status(403)
        .json({ error: "Not authorized to view this shipment's timeline" });
      return;
    }
...
export const closeShipment = async (
  ...
  try {
    ...
    if (error instanceof z.ZodError) {
      res
        .status(400)
        .json({ error: "Invalid POD close data", details: error.errors });
      return;
    }
...
export const getOverdueShipments = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getLogisticsStatusUpdates = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const createLogisticsStatusUpdate = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getStatusUpdatesByShipment = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    // Get existing shipment for authorization
    const existingShipment = await storage.getLogisticsShipment(
      req.params.shipmentId
    );
    if (!existingShipment) {
      res.status(404).json({ error: "Shipment not found" });
    ...
    if (
      ...
      res.status(403).json({
        error: "Not authorized to view this shipment's status updates",
      });
      return;
    }

    const updates = await storage.getStatusUpdatesByShipment(
      req.params.shipmentId
    );
    res.json(updates);
  } catch (error) {
    res
      .status(500)
      .json({ error: "Failed to fetch status updates for shipment" });
  }
};
...
export const getLogisticsCheckpoints = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const createLogisticsCheckpoint = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (error instanceof z.ZodError) {
      res
        .status(400)
        .json({ error: "Invalid checkpoint data", details: error.errors });
      return;
    }
...
export const getCheckpointsByShipment = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (
      ...
      res
        .status(403)
        .json({ error: "Not authorized to view this shipment's checkpoints" });
      return;
    }

    const checkpoints = await storage.getCheckpointsByShipment(
      req.params.shipmentId
    );
    res.json(checkpoints);
  } catch (error) {
...
export const getLogisticsHealth = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getLogisticsTasks = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getLogisticsTask = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const createLogisticsTask = async (
  ...
  try {
    ...
    if (req.user!.role === "employee" && taskData.assignedTo !== req.user!.id) {
      res
        .status(403)
        .json({ error: "Employees can only create tasks for themselves" });
      return;
    }
...
export const updateLogisticsTask = async (
  ...
  try {
    ...
    const updatedTask = await storage.updateLogisticsTask(
      req.params.id,
      validatedData
    );
...
export const getLogisticsAttendance = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const checkInLogistics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    await storage.createActivity({
      ...
      details: `Checked in for logistics at ${
        validatedData.location || "GPS location"
      }. Accuracy: ${validatedData.accuracy}m`,
    });
...
export const checkOutLogistics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (existingAttendance.userId !== req.user!.id) {
      res
        .status(403)
        .json({ error: "Not authorized to modify this attendance record" });
      return;
    }
    ...
    const updateData = {
      ...
      workDescription:
        validatedData.workDescription || existingAttendance.workDescription,
    ...
    await storage.createActivity({
      ...
      entityType: "logistics_attendance",
      entityId: req.params.id,
      details: `Checked out from logistics at ${
        validatedData.location || "GPS location"
      }. Tasks: ${validatedData.taskCount || 0}, Deliveries: ${
        validatedData.deliveriesCompleted || 0
      }`,
    });
...
export const getLogisticsAttendanceToday = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch today's logistics attendance" });
  }
};

// Photo upload URL generation for attendance
export const generateAttendancePhotoUploadUrl = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    const objectPath = `attendance-photos/${validatedData.attendanceId}/${
      validatedData.photoType
    }-${Date.now()}-${validatedData.fileName}`;
...
export const updateAttendancePhoto = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!["check-in", "check-out"].includes(photoType)) {
      res
        .status(400)
        .json({ error: "Photo type must be 'check-in' or 'check-out'" });
      return;
    }
    ...
    if (attendanceRecord.userId !== req.user!.id) {
      res
        .status(403)
        .json({ error: "Not authorized to update this attendance record" });
      return;
    }
...
export const getLogisticsAttendanceMetrics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch logistics attendance metrics" });
  }
};
...
export const getLogisticsDashboardMetrics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch logistics dashboard metrics" });
  }
};

export const getDailyShipmentsReport = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getVendorPerformanceReport = async (
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch vendor performance report" });
  }
};

export const getShipmentVolumeMetrics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const getDeliveryPerformanceMetrics = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to fetch delivery performance metrics" });
  }
};
...
export const exportDailyShipmentsReport = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    if (!from || !to) {
      res
        .status(400)
        .json({ error: "Date range (from/to) parameters are required" });
      return;
    }
    ...
    while (currentDate <= endDate) {
      const dailyReport = await storage.getDailyShipmentsReport(
    ...
    await generateExportFile(res, {
      data,
      filename: `daily-shipments-${startDate.toISOString().split("T")[0]}-to-${
...
export const exportVendorPerformanceReport = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
    ...
    await generateExportFile(res, {
      ...
      filename: `vendor-performance-${
        new Date(from as string).toISOString().split("T")[0]
      }-to-${new Date(to as string).toISOString().split("T")[0]}`,
      format: format as string,
    ...
    res
      .status(500)
      .json({ error: "Failed to export vendor performance report" });
  }
};

export const exportVolumeReport = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const exportComprehensiveReport = async (
  ...
  try {
    ...
    await generateExportFile(res, {
      data,
      filename: `comprehensive-logistics-report-${
        new Date(from as string).toISOString().split("T")[0]
      }-to-${new Date(to as string).toISOString().split("T")[0]}`,
      format: format as string,
...
async function generatePDFExport(
  ...
  if (type === "comprehensive") {
    ...
    if (data.dashboardMetrics) {
      ...
      doc.setFontSize(10);
      doc.text(
        `Total Shipments: ${metrics.totalShipments || 0}`,
        20,
        yPosition
      );
      doc.text(
        `Delivered: ${metrics.deliveredShipments || 0}`,
        20,
        yPosition + 10
      );
...
export const generatePodUploadUrl = async (
  req: AuthenticatedRequest,
  res: Response
): Promise<void> => {
  try {
...
export const registerLogisticsRoutes = (
  app: Express,
  middleware: LogisticsRouteOptions
): void => {
  const {
    requireAuth: auth,
    requireLogisticsAccess: logisticsAccess,
    checkOwnership,
  } = middleware;
  ...
  const logisticsRoutes: LogisticsRoute[] = [
    ...
    // ==========================================
    {
      ...
      handler: getLogisticsShipments,
    },
    {
      method: "get",
      path: "/api/logistics/shipments/active",
      middlewares: ["requireAuth"],
      handler: getActiveShipments,
    },
    {
      method: "get",
      path: "/api/logistics/shipments/overdue",
      middlewares: ["requireAuth"],
      handler: getOverdueShipments,
    },
    {
      method: "get",
      path: "/api/logistics/shipments/search",
      middlewares: ["requireAuth"],
      handler: searchShipments,
    },
    {
      method: "get",
    ...
    },
    {
      ...
      path: "/api/logistics/shipments",
      middlewares: ["requireAuth"],
      handler: createLogisticsShipment,
    },
    {
      method: "put",
      path: "/api/logistics/shipments/:id",
      middlewares: ["requireAuth", "checkOwnership:shipment"],
      handler: updateLogisticsShipment,
    },
    {
      method: "delete",
      path: "/api/logistics/shipments/:id",
    ...
    {
      method: "get",
      path: "/api/logistics/shipments/:id/timeline",
      middlewares: ["requireAuth"],
      handler: getShipmentTimeline,
    },
    {
      method: "post",
      path: "/api/logistics/shipments/:id/close",
      middlewares: ["requireAuth", "checkOwnership:shipment"],
      handler: closeShipment,
    },
    ...
    {
      method: "post",
      path: "/api/logistics/pod/upload-url",
      middlewares: ["requireAuth"],
      handler: generatePodUploadUrl,
    },
    {
      method: "post",
      path: "/api/logistics/attendance/photo/upload-url",
      middlewares: ["requireAuth"],
      handler: generateAttendancePhotoUploadUrl,
    },
    ...
    {
      method: "get",
      path: "/api/logistics/status-updates",
      middlewares: ["requireAuth"],
      handler: getLogisticsStatusUpdates,
    },
    {
      method: "post",
      path: "/api/logistics/status-updates",
      middlewares: ["requireAuth"],
      handler: createLogisticsStatusUpdate,
    },
    {
      method: "get",
      path: "/api/logistics/status-updates/shipment/:shipmentId",
      middlewares: ["requireAuth"],
      handler: getStatusUpdatesByShipment,
    },
    ...
    {
      method: "get",
      path: "/api/logistics/checkpoints",
      middlewares: ["requireAuth"],
      handler: getLogisticsCheckpoints,
    },
    {
      method: "post",
      path: "/api/logistics/checkpoints",
      middlewares: ["requireAuth"],
      handler: createLogisticsCheckpoint,
    },
    {
      method: "get",
      path: "/api/logistics/checkpoints/shipment/:shipmentId",
      middlewares: ["requireAuth"],
      handler: getCheckpointsByShipment,
    },
    ...
    {
      method: "get",
      path: "/api/logistics/tasks",
      middlewares: ["requireAuth"],
      handler: getLogisticsTasks,
    },
    {
      method: "get",
      path: "/api/logistics/tasks/:id",
      middlewares: ["requireAuth"],
      handler: getLogisticsTask,
    },
    {
      method: "post",
      path: "/api/logistics/tasks",
      middlewares: ["requireAuth"],
      handler: createLogisticsTask,
    },
    {
      method: "put",
      path: "/api/logistics/tasks/:id",
      middlewares: ["requireAuth"],
      handler: updateLogisticsTask,
    },
    {
      method: "delete",
      path: "/api/logistics/tasks/:id",
      middlewares: ["requireAuth"],
      handler: deleteLogisticsTask,
    },
    ...
    {
      method: "get",
      path: "/api/logistics/attendance",
      middlewares: ["requireAuth"],
      handler: getLogisticsAttendance,
    },
    {
      method: "get",
      path: "/api/logistics/attendance/today",
      middlewares: ["requireAuth"],
      handler: getLogisticsAttendanceToday,
    },
    {
      method: "get",
      path: "/api/logistics/attendance/metrics",
      middlewares: ["requireAuth"],
      handler: getLogisticsAttendanceMetrics,
    },
    {
      method: "post",
      path: "/api/logistics/attendance/check-in",
      middlewares: ["requireAuth"],
      handler: checkInLogistics,
    },
    {
      method: "put",
      path: "/api/logistics/attendance/:id/check-out",
      middlewares: ["requireAuth"],
      handler: checkOutLogistics,
    },
    {
      method: "post",
      path: "/api/logistics/attendance/photo/upload-url",
    ...
    {
      method: "put",
      path: "/api/logistics/attendance/:id/photo",
      middlewares: ["requireAuth"],
      handler: updateAttendancePhoto,
    },
    ...
    // ==========================================
    {
      ...
      middlewares: ["requireAuth"],
      handler: getLogisticsDashboardMetrics,
    },
    {
      method: "get",
      path: "/api/logistics/reports/daily",
      middlewares: ["requireAuth"],
      handler: getDailyShipmentsReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/delivery-time",
      middlewares: ["requireAuth"],
      handler: getAverageDeliveryTime,
    },
    {
      method: "get",
      path: "/api/logistics/reports/vendor-performance",
      middlewares: ["requireAuth"],
      handler: getVendorPerformanceReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/volume",
      middlewares: ["requireAuth"],
      handler: getShipmentVolumeMetrics,
    },
    {
      method: "get",
      path: "/api/logistics/reports/performance",
      middlewares: ["requireAuth"],
      handler: getDeliveryPerformanceMetrics,
    },

    // Export routes
    {
      method: "get",
      path: "/api/logistics/reports/daily/export",
      middlewares: ["requireAuth"],
      handler: exportDailyShipmentsReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/vendor-performance/export",
      middlewares: ["requireAuth"],
      handler: exportVendorPerformanceReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/volume/export",
      middlewares: ["requireAuth"],
      handler: exportVolumeReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/performance/export",
      middlewares: ["requireAuth"],
      handler: exportPerformanceReport,
    },
    {
      method: "get",
      path: "/api/logistics/reports/comprehensive/export",
      middlewares: ["requireAuth"],
      handler: exportComprehensiveReport,
    },
    ...
    {
      method: "get",
      path: "/api/logistics/health",
      middlewares: [],
      handler: getLogisticsHealth,
    },
  ];

  console.log(
    `📋 Registering ${logisticsRoutes.length} logistics routes from registry...`
  );
      ...
      if (mwName === "requireAuth") {
        ...
        middlewares.push(
          checkOwnership
            ...
            : checkLogisticsOwnership(entityType)
        );
      }
    }
  ...
  console.log(
    `✅ Logistics routes registry registration complete: ${logisticsRoutes.length} routes`
  );
  ...
  if (logisticsRoutes.length !== EXPECTED_LOGISTICS_ROUTE_COUNT) {
    console.warn(
      `⚠️  Logistics route count mismatch: Expected ${EXPECTED_LOGISTICS_ROUTE_COUNT}, found ${logisticsRoutes.length}`
    );
  }
  ...
  criticalRoutes.forEach((route) => {
    ...
    const found = logisticsRoutes.some(
